...
#ifndef GIT_ENOTIMPLEMENTED
#define GIT_ENOTIMPLEMENTED GIT_ERROR
#endif
...
int sqlite_backend__write(git_odb_backend *_backend, const git_oid *oid_, const void *data, size_t len, git_otype type)
{
	int error = SQLITE_ERROR;
	sqlite_backend *backend = NULL;
	...
	if (sqlite3_bind_text(backend->st_write, 1, (char *)oid->id, 20, SQLITE_TRANSIENT) == SQLITE_OK &&
