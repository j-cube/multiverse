...
int memcached_backend__read_header(size_t *len_p, git_otype *type_p, git_odb_backend *_backend, const git_oid *oid)
{
	...
	char *type_key = NULL, *size_key = NULL;
	git_otype *type_buffer = NULL;
	size_t type_key_len, type_value_len, size_key_len, *size_value = NULL, size_value_len;
	...
	type_buffer = (git_otype *) memcached_get(backend->db, type_key, type_key_len, &type_value_len, &type_flags, &ret);
	if (! type_p) {
...
int memcached_backend__write(git_odb_backend *_backend, const git_oid *oid_, const void *data, size_t len, git_otype type)
{
	memcached_backend *backend;
	memcached_return ret = 0;
	git_oid oid_data, *oid;
